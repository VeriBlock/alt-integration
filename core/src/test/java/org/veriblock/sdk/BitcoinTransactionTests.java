...
public class BitcoinTransactionTests {
    ...
    public void parse() {
        Random random = new Random(100L);
        byte[] bytes = new byte[243];
        random.nextBytes(bytes);

        BitcoinTransaction input = new BitcoinTransaction(bytes);
        byte[] serialized = SerializeDeserializeService.serialize(input);
        BitcoinTransaction deserialized = SerializeDeserializeService.parseBitcoinTransaction(ByteBuffer.wrap(serialized));
