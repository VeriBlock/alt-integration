...
public class MerkleTree {
    ...
    public static MerkleTree buildTree(BlockMetaPackage meta, List<VeriBlockPoPTransaction> popTransactions, List<VeriBlockTransaction> normalTransactions) {
        ...
        List<MerkleNode> pop = popTransactions.stream().map(tx -> new MerkleNode(tx.getId())).collect(Collectors.toList());
        MerkleNode popTree = build(pop);

        List<MerkleNode> regular = normalTransactions.stream().map(tx -> new MerkleNode(tx.getId())).collect(Collectors.toList());
