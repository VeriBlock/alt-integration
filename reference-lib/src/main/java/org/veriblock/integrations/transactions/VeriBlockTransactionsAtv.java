...
public class VeriBlockTransactionsAtv {
    ...
    public static VeriBlockTransaction createAtv(PublicationData altData) throws SignatureException, InvalidKeyException, NoSuchAlgorithmException {
    ...
    public static VeriBlockTransaction createAtv() throws SignatureException, InvalidKeyException, NoSuchAlgorithmException {
        PublicationData data = new PublicationData(1, new byte[] {}, new byte[] {}, new byte[] {});
        VeriBlockTransaction tx = createAtv(data);
        return tx;
    }
    ...
    public static AltPublication createAtvPublicationWithContainingBlockAttached(VeriBlockBlock containingBlockReference, VeriBlockTransaction tx) throws SignatureException {
        ...
        AltPublication publication = new AltPublication(
                tx,
                tree.getMerklePath(tx.getId(), 0, true),
                containingBlock,
                Collections.emptyList());
        return publication;
    }
    ...
    public static AltPublication createAtvPublicationNotAttached(VeriBlockTransaction tx, int veriBlockHeight) {
        ...
        VeriBlockMerklePath proof = tree.getMerklePath(tx.getId(), 0, true);
        AltPublication publication = new AltPublication(
                tx,
                proof,
                containingBlock,
    ...
    public static AltPublication createAtvPublicationNotAttached() throws SignatureException, InvalidKeyException, NoSuchAlgorithmException {
        VeriBlockTransaction tx = createAtv();
        AltPublication publication = createAtvPublicationNotAttached(tx);
        return publication;
    }
    
    public static AltPublication createAtvPublicationNotAttached(PublicationData pubData) throws SignatureException, InvalidKeyException, NoSuchAlgorithmException {
